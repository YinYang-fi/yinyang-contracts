...
contract YinDistributor {
    ...
    struct PoolInfo {
        ...
        uint256 accYinPerShare; // Accumulated YANGs per share, times 1e18.
    }
    ...
    uint256[] public epochTotalRewards = [200000 ether, 400000 ether, 300000 ether, 300000 ether];
    ...
    uint[5] public epochYinPerBlock;
    ...
    constructor(
        ...
        if (_yin != address(0)) yin = IBEP20(_yin);
        ...
        for (i = 0; i <= 3; ++i) {
            epochYinPerBlock[i] = epochTotalRewards[i].div(BLOCKS_PER_WEEK);
        }
        epochYinPerBlock[4] = 0;
        operator = msg.sender;
        creator = msg.sender;
    }

    modifier onlyOperator() {
        require(operator == msg.sender, "YinDistributor: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IBEP20 _lpToken) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].lpToken != _lpToken, "YinDistributor: existing pool?");
        }
    }
    ...
    function add(
        ...
        require(_depositFee <= 10000, "YinDistributor: invalid deposit fee");
    ...
    function getGeneratedReward(uint256 _from, uint256 _to) public view returns (uint256) {
        for (uint8 epochId = 4; epochId >= 1; --epochId) {
            if (_to >= epochEndBlocks[epochId - 1]) {
                if (_from >= epochEndBlocks[epochId - 1]) return _to.sub(_from).mul(epochYinPerBlock[epochId]);
                uint256 _generatedReward = _to.sub(epochEndBlocks[epochId - 1]).mul(epochYinPerBlock[epochId]);
                if (epochId == 1) return _generatedReward.add(epochEndBlocks[0].sub(_from).mul(epochYinPerBlock[0]));
                for (epochId = epochId - 1; epochId >= 1; --epochId) {
                    if (_from >= epochEndBlocks[epochId - 1]) return _generatedReward.add(epochEndBlocks[epochId].sub(_from).mul(epochYinPerBlock[epochId]));
                    _generatedReward = _generatedReward.add(epochEndBlocks[epochId].sub(epochEndBlocks[epochId - 1]).mul(epochYinPerBlock[epochId]));
                }
                return _generatedReward.add(epochEndBlocks[0].sub(_from).mul(epochYinPerBlock[0]));
            }
        }
        return _to.sub(_from).mul(epochYinPerBlock[0]);
    }
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 _yinReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accYinPerShare = accYinPerShare.add(_yinReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accYinPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardBlock, block.number);
            uint256 _yinReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accYinPerShare = pool.accYinPerShare.add(_yinReward.mul(1e18).div(lpSupply));
        }
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accYinPerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeYinTransfer(_sender, _pending);
        ...
        if (_amount > 0) {
            ...
            if(pool.depositFee > 0 && pool.depositFee != 10000) {
        ...
        user.rewardDebt = user.amount.mul(pool.accYinPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        address _sender = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.depositFee != 10000, "YinDistributor: donation pool");
        UserInfo storage user = userInfo[_pid][_sender];
    ...
    // Safe yin transfer function, just in case if rounding error causes pool to not have enough Yin.
    function safeYinTransfer(address _to, uint256 _amount) internal {
        uint256 _yinBal = yin.balanceOf(address(this));
        if (_yinBal > 0) {
            if (_amount > _yinBal) {
                yin.safeTransfer(_to, _yinBal);
            } else {
                yin.safeTransfer(_to, _amount);
            }
        }
    }
